...
export default function HomePageHorizontal({
  data,
}: {
  ...
  const laptopContainerPositionFromTop = laptopContRef.current
    ? laptopContRef.current.getBoundingClientRect().top + 20
    : 40
  /* End */
      ...
      dragInstance.current = Draggable.create(gsapDragRef.current, {
        ...
        onDrag: () => {
          const proportionalMovement =
            (ref.current.getBoundingClientRect().width - x) / scrollBarLength
          lenisCurrent.scrollTo(
            dragInstance.current[0].y * proportionalMovement,
          )
  ...
  return (
    <div
      ref={laptopTotalContainerRef}
      className="xl:h-[100vh] xl:absolute top-0  xl:flex items-center xl:justify-center"
    >
      <div className="fixed relative right-[28px] xl:block hidden ">
        <div
          style={{ top: laptopContainerPositionFromTop }}
          ref={refCallback}
          className={`grabb w-fit right-[26px] cursor-grab 3xl:right-[25px]  shadow-lg line fixed sb  z-30 rounded-sm`}
        >
          <Image
            src={ScrollBarIcon.src}
            width={26}
            height={100}
            className="h-fit 2xl:w-[24px] 3xl:w-[45px] grabb cursor-grab"
            alt="Scrollbar"
          ></Image>
        </div>
        <div
          style={{
            top: laptopContainerPositionFromTop,
            height: scrollBarLength + 0.0388 * y,
          }}
          className={`3xl:w-[40px] w-[20px] bg-bl/[0.11] right-[28px] top-[${Math.floor(laptopContainerPositionFromTop)}px] z-10 fixed`}
        ></div>
      </div>
      ...
      <div
        ref={laptopContRef}
        className="xl:flex hidden flex-col  h-fit py-auto w-fit "
      >
        <div></div>
        <motion.div
          ref={ref}
          ...
          className="grid h-full  w-fit  px-4 grid-rows-2 z-0 grid-flow-col gap-6 3xl:gap-8"
        >
          <ReelVideo
            url={process.env.NEXT_PUBLIC_REEL_DISPLAY_VIDEO || ''}
          ></ReelVideo>
          {finalModifiedArray &&
            finalModifiedArray.map((i, k) => {
              ...
              return (
                <section
                  ...
                  draggable={false}
                  key={k}
                  className=" h-full pointerr relative cursor-pointer "
                >
                  {
                    <ProjectCard
                      slug={i.slug || '/'}
                      animate={k === finalModifiedArray.length - 1}
